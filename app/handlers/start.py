...
async def start_command_handler(message: Message, state: FSMContext) -> None:
    ...
    if user_id is None:
        await message.answer("Не вдалося визначити ваш Telegram ID. Спробуйте команду /start ще раз.")
        return
...
async def receive_phone_handler(message: Message, state: FSMContext) -> None:
    ...
    if auth_agent is None:
        ...
        await message.answer(
            "1С не підтвердила ваш номер. Перевірте контакт або зверніться до керівника.",
            reply_markup=build_request_contact_keyboard(),
        )
        return
    ...
    try:
        ...
        await message.answer("Не вдалося зберегти профіль. Спробуйте ще раз або зверніться до адміністратора.")
        return
    ...
    await message.answer(
        f"Авторизація успішна. Вітаю, {auth_agent.name}.",
        reply_markup=build_main_keyboard(),
    )
...
async def waiting_for_code_deprecated_handler(message: Message, state: FSMContext) -> None:
    ...
    await state.set_state(AuthStates.waiting_for_phone)
    await message.answer(
        "Додатковий код більше не потрібен. Надішліть свій контакт кнопкою нижче.",
        reply_markup=build_request_contact_keyboard(),
    )
